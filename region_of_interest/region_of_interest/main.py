...
from region_of_interest.semantic_pre_processing.bboxes_calculation import BboxCalculation
...
# GLOBAL PARAMETERS
# Paths
CONFIGS = yaml.load(Path("./externals/configs.yaml").read_text(), Loader=yaml.Loader)
PATHS = CONFIGS['PATHS']
...
def get_bboxes(semantic_field_pt: PosixPath) -> List[o3d.geometry.AxisAlignedBoundingBox]:
    bbox_calculator = BboxCalculation(semantic_field_pt)
...
def main():
    ...
    # Obtaining bboxes
    bboxes = get_bboxes(PATHS["TORCH"]["SEMANTIC_PC_PATH"])
